...
*   **Grade System Integration (급수 통합 시스템 개발)**:
    *   현재 구현된 **연령대 통합(`integrated` mode)** 로직에 이어, 특정 **급수(Grade)를 하나로 묶어 운영**할 수 있는 아키텍처 고도화.
    *   S/A급 또는 B/C급 등을 하나의 부문으로 통합하여 대진표를 구성하고, 유연하게 참여 인원을 관리하는 운영 시스템 구축.
...
*   **Full Firebase Integration (Vercel API 이관)**:
    *   **목적:** 네이버 로그인 시 발생하는 지연 시간(Latency) 개선 및 인프라 단일화.
    *   **내용:** 현재 Vercel에서 처리 중인 네이버 OAuth 토큰 교환 및 프로필 조회 로직을 Firebase Cloud Functions(서울 리전)로 이관. 
    *   **최적화:** 두 단계로 나뉜 API 요청을 단일 함수로 통합하여 로그인 속도 극대화.
